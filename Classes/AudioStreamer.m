...
@synthesize vbr;
@synthesize preloadCallback, successCallback, failedCallback, seekCallback;
@synthesize canSeek=queueStarted;
...
// isLocalFileUrl
//
// Determines whether the specified Url is pointing to a local resource
//
- (bool) isLocalFileUrl
{
...
- (BOOL)openReadStream
{
	@synchronized(self)
	{
		...
		if ([self isLocalFileUrl]) {
			NSLog(@"Local File Resource detected for reading");
			stream = CFReadStreamCreateWithFile(NULL, (CFURLRef)url);

			// Seek into the file at a byte offset if required
			if (fileLength > 0 && seekByteOffset > 0)
			{
				NSLog(@"Warning: Not able to seek in a local stream");
				discontinuous = vbr;
			}
		} else {
			NSLog(@"Remote Url detected for reading");
	...
	do
	{
		...
		@synchronized(self) {
			if (seekWasRequested) {
                ...
                [self raiseBlockCallback:self.seekCallback];
				seekWasRequested = NO;
			}
		}
		...
		//
		if (buffersUsed == 0 && self.state == AS_PLAYING && isFilled == NO)
		{
...
- (void)prepareSeek:(double)newSeekTime
{
...
- (BOOL)seekToTime:(double)newSeekTime:(SimpleBlock)successBlock
{
    if (queueStarted == NO)
    {
        ...
        @synchronized(self)
        {
            ...
            self.seekCallback = successBlock;
        }
        return YES;
    }
}
...
//					stopReason = AS_STOPPING_EOF;
	...
	{
		if (![self isLocalFileUrl] && !httpHeaders)
		{
			CFTypeRef message =
...
- (void)enqueueBuffer
{
	@synchronized(self)
	{
			...
			(state == AS_STOPPED && stopReason == AS_STOPPING_TEMPORARILY))
		{
			...
			//
			if (state == AS_FLUSHING_EOF || buffersUsed >= kMinAQBufs - 1)
			{
				if (preloadRequested) {
					// Preload completed
					preloadRequested = NO;
...
#ifdef LOG_QUEUED_BUFFERS
    NSLog(@"Buffer %i completed", bufIndex);
#endif
	if (bufIndex == -1)
	{
	...
	propertyID:(AudioQueuePropertyID)inID
{
	...
	@synchronized(self)
	{
		if (inID == kAudioQueueProperty_IsRunning)
		{
            ...
            if (state == AS_STOPPING && isRunning == NO)
            {
                isFilled = NO;
                queueStarted = NO;
                self.state = AS_STOPPED;
